...
contract HardcapCrowdsale is Ownable {
  ...
  uint256 private constant TEAM_PERCENTAGE = 0;
  uint256 private constant PLATFORM_PERCENTAGE = 0;
  uint256 private constant CROWDSALE_PERCENTAGE = 0;

  uint256 private constant MIN_TOKENS_TO_PURCHASE = 0 * 10**18;

  uint256 private constant ICO_TOKENS_CAP = 0 * 10**24;

  uint256 private constant FINAL_CLOSING_TIME = 0;
  ...
  function HardcapCrowdsale(address _wallet, address _platform, HardcapToken _token) public {
      ...
      phases[1] = Phase(0 * 10**24, 0);
      phases[2] = Phase(0 * 10**24, 0);
      // TBD
  }
